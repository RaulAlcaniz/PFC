class PaymentsController < ApplicationController
  ...
  before_action :set_person, only: [:new, :create, :show, :edit]
  ...
  def create
    ...
    if @payment.save
      update_enrolments  # Posiblemente se puede quitar
      @payment.unverified!
  ...
  private
  def payment_params
    params.require(:payment).permit(:amount, :comment, :receipt, :status)
  end
  ...
  def set_enrolments
    ...
    # else #@payment
    #   Enrolment.all.find_all{|enrolment|
    #     @enrolments << enrolment if (enrolment.payment_id == @payment.id)
    #   }
    end
  end
  ...
  def update_enrolments
    # raise 'You have dogs already enrolled in this exhibition and you have paid for them.' if @enrolments.empty?

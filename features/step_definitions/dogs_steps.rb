...
def get_abbreviations text
  text.strip!
  text.sub! 'J', 'Junior'
  text.sub! 'I', 'Intermediate'
  text.sub! 'O', 'Open'
  text.sub! 'W', 'Working'
  text.sub! 'CH', 'Champion'
end

Given(/^there are these payments for "(.*?)" in "(.*?)":$/) do |clients, exhib_name, prices_table|
  #prices_table.map_column!('Class') { |cell| get_abbreviations(cell)}
  ...
  @classes.each do |class_name, class_group|
    @deadlines["#{exhib_name}"].each do |date|
      deadline_prices = []
      class_group.each { |j| deadline_prices << j[date[:name]] }
  ...
  grouped_classes.each do |k|
    k[:classes].each {|p| get_abbreviations p}
  end
  @groups = Hash.new
  @groups['Exp. Canina de Cieza'] = grouped_classes

  #puts @grouped_classes.hashes
  ...
  #puts @tax
  #@tax[:prices].merge!(:partners => prices_collect) if clients == 'partners'
  #@tax[:prices].merge!(:nopartners => prices_collect) if clients == 'partners'
  #@tax[:prices].store :partners => prices_collect if clients == 'partners'
  #@tax[:prices].store :nopartners => prices_collect if clients == 'partners'
